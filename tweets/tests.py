...
class TweetTests(TestCase):
    ...
    def test_create_photo(self):
        ...
        self.assertEqual(self.tweet.tweetphoto_set.count(), 1) #tweetphoto_set() 反查机制

    def test_cache_tweet_in_redis(self):
        tweet = self.create_tweet(self.user1)
        conn = RedisClient.get_connection()
        serialized_data = DjangoModelSerializer.serialize(tweet)
        conn.set(f'tweet:{tweet.id}', serialized_data) # store
        data = conn.get(f'tweet:not_exists')
        self.assertEqual(data, None)

        data = conn.get(f'tweet:{tweet.id}') # read
...
class TweetServiceTests(TestCase):
    ...
    def test_get_user_tweets(self):
        ...
        # test cache miss (when cache missed, will create cache)
    ...
    def test_create_new_tweet_before_get_cached_tweets(self):
        ...
        # because we cleared cache, so the key does not exist in the cache
        self.assertEqual(conn.exists(key), False)
        # when creating new tweet, if there is no tweet cache for this user id,
        # then it will create cache for it.
